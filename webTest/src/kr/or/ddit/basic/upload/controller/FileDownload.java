...
public class FileDownload extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// => 실제 저장된 파일 명을 이용하여 생성한다
		File downFile = new File(f, fvo.getSave_file_name());
		
		if(downFile.exists()) {	//해당 파일이 있을 때 처리...
			//Download할 때의 Content 설정
			response.setContentType("application/octet-stream; charset=utf-8");
			
			//응답(Response)객체의 Header값 설정한다
			String headerKey = "content-disposition";
			
			//이 Header의 value값으로는 클라이언트에 저장될 파일명을 지정해 주면 된다.
			// ==> 원래의 파일명으로 지정한다.
			
			//방법 1 => 파일명에 한글이 포함 안되었을때 (예전방식)
			//String headerValue = "attachment; filename=\"" + fvo.getOrigin_file_name() + "\"" ;
			
			//방법 2 => 파일명에 한글이 포함된 경우
			String headerValue = "attachment; filename*=UTF-8''" + URLEncoder.encode(fvo.getOrigin_file_name(), "utf-8")
			.replaceAll("\\+", "%20");
