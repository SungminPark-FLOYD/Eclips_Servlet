...
/*
 *  - Servlet 3.0이상에서 파일 업로드를 하려면 서블릿에 @MultipartConfig 어노테이션을 설정해야 한다..
 *  
 *  - @MultipartConfig 어노테이션의 설정해야할 속성들
 *  1) location : 업로드한 파일이 임시적으로 저장될 경로 지정(기본값 : "")
 *  2) fileSizeThreshold : 이 속성에 지정한 값보다 큰 파일이 전송되면 location속성에 지정한 임시 경로에 저장된다.(단위 : byte, 기본값 : 0)
 *  3) maxFileSize : 1개 파일의 최대 크기 설정 (단위 : byte, 기본값 : -1(무제한))
 *  4) maxRequestSize : 서버로 전송되는 Request데이터 전체의 최대 크기 설정
 *  					(모든 파일의 크기의 합 _ formData)
 *  					(단위 : byte, 기본값 : -1(무제한))
 *   */
...
public class FileUpload extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for(Part part : request.getParts()) {
			...
			//찾은 파일이름이 null이면 파일이 아닌 일반 파라미터 데이터
			if(fileName != null) {	//파일인지 검사
				//1개의 파일 정보가 저장 될 FileInfoVo객체 생성
				FileInfoVo fvo = new FileInfoVo();
				
				fvo.setFile_writer(userName); 	//작성자를 VO객체에 저장
				fvo.setOrigin_file_name(fileName); 	//원래의 파일 명을 Vo객체에 저장
				
				//실제 저장되는 파일 이름이 중복되는 것을 방지하기 위해서 uuid객체를 
				//이용하여 저장할 파일명을 만든다.
				String saveFileName = UUID.randomUUID().toString() + "_" + fileName;
				
				//새로만든 파일명을 Vo객체에 '저장 파일명'으로 저장
	...
	/*
	 * 1) 파일이 아닌 일반 데이터일 경우
	 * -------------------213askljdlaksjd => Part들을 구분하는 구분선
