...
/*
 * Servlet동작 과정..(Servlet의 LifeCycle)
 ...
 * 3. Servlet Container는 개별 요청을 처리할 쓰레드를 생성하여 해당 서블릿객체의 
 * 	service()메서드를 호출한다 (이 때 HttpServletRequest객체와 HttpResponse객체를 생성하여
 * 	파라미터로 넘겨준다.)
 * 4. service()메서드는 전송방식(Get, Post등)을 체크하여 적절한 메서드를 호춣나다
 * 	(doGet, doPost, doPut, doDelete등...)
 * 5. 요청과 요청에 대한 응답이 모두 완료되면 HttpServletRequest객체와
 * 	HttpServletPesponse객체는 자동으로 소멸된다.
...
public class ServletTest03 extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//방법2) 클라이언트의 전송방식을(get,Post 등) 구분해서 직접 메서드 호출하기
